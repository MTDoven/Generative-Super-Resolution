...
class Flux2(nn.Module):
    ...
    def convert_layernorm_elementwise_affine_(self, verbose: bool = True) -> int:
        """Convert LayerNorm(elementwise_affine=False) to affine=True with identity init.

        This keeps the initial forward behavior equivalent to affine=False:
        y = LayerNorm(x) * 1 + 0.
        """
        converted = 0
        ...
        for name, module in list(self.named_modules()):
            if isinstance(module, nn.LayerNorm) and module.weight is None:
                replacement = nn.LayerNorm(
                    module.normalized_shape,
                    eps=module.eps,
                    elementwise_affine=True,
                )
                if ref_device is not None or ref_dtype is not None:
                    replacement = replacement.to(device=ref_device, dtype=ref_dtype)

                with torch.no_grad():
                    replacement.weight.fill_(1.0)
                ...
                if verbose:
                    print(f"[INFO] {name}: set elementwise_affine=True (identity init)")
